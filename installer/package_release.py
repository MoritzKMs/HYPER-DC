...
with zipfile.ZipFile(release/'HYPER-DC-0.2.0-source.zip','w',zipfile.ZIP_DEFLATED) as z:
 for p in root.rglob('*'):
  rel=p.relative_to(root)
  if p.is_file() and not any(x in ignore for x in rel.parts) and p.name != 'payload.zip': z.write(p,'HYPER-DC/'+str(rel).replace('\\','/'))
with zipfile.ZipFile(release/'HYPER-DC-0.2.0-dist.zip','w',zipfile.ZIP_DEFLATED) as z:
 for p in (root/'dist').iterdir():
  if p.is_file(): z.write(p,p.name)
 z.write(root/'LICENSE','LICENSE')
names=['HyperDCInstaller.exe','HYPER-DC-0.2.0-source.zip','HYPER-DC-0.2.0-dist.zip']
