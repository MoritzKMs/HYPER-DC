...
export async function hasVoiceBackup() {
    return !!await DataStore.get<VoiceBackup>(backupKey);
}

export async function restoreVoiceSetup() {
    const saved = await DataStore.get<VoiceBackup>(backupKey);
    if (!saved) return;
    ...
    await DataStore.del(backupKey);
}

export async function applyVoiceSetup(devices: MediaDeviceInfo[], selectedSink: string) {
    ...
    if (!await hasVoiceBackup()) await DataStore.set(backupKey, {
